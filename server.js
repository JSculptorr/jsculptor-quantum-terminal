...
mongoose.connect(process.env.MONGO_URI)
    .then(() => console.log(">> [BINANCE_SCALPER]: DB_CONNECTED_STABLE"))
...
// API Истории (24/7 доступ к сигналам)
...
class MarketState {
    constructor() {
        ...
        // Новое для Скальпера: Скорость ленты
        this.tradeCount = 0;
        this.velocity = 0; 
    }
...
// --- SNIPER SCALP ENGINE v7.1 (АНТИ-СПАМ И ТРЕНД) ---
async function analyzeMarket(s) {
    ...
    // 1. ТРЕНДОВЫЙ ФИЛЬТР (ГЛАВНЫЙ ЦЕНЗОР)
    // Мы входим только в сторону тренда по EMA 21
    const currentTrend = d.price > d.ema21 ? "LONG" : "SHORT";

    // 2. СКОРИНГ (Набираем баллы только если импульс совпадает с трендом)
    const last = d.candles[d.candles.length - 1];
    if (last && last.volume > d.volumeSMA * 2.0) {
        score += 30; reasons.push("Volume Spike");
    }

    if (d.velocity > 60) { 
        score += 20; reasons.push("High Tape Velocity");
    }

    // Ликвидность и SFP (только по тренду)
    if (currentTrend === "LONG" && d.swings.low && d.price < d.swings.low && cvd > 0) {
        score += 30; reasons.push("Bullish Liquidity Sweep");
    } else if (currentTrend === "SHORT" && d.swings.high && d.price > d.swings.high && cvd < 0) {
        score += 30; reasons.push("Bearish Liquidity Sweep");
    }

    // Дисбаланс стакана в сторону тренда
    const imb = (d.depth.bids - d.depth.asks) / (d.depth.bids + d.depth.asks);
    if (currentTrend === "LONG" && imb > 0.35) {
        score += 20; reasons.push("Bids Pressure");
    } else if (currentTrend === "SHORT" && imb < -0.35) {
        score += 20; reasons.push("Asks Pressure");
    }

    // CVD Подтверждение
    if ((currentTrend === "LONG" && cvd > 0) || (currentTrend === "SHORT" && cvd < 0)) {
        score += 15; reasons.push("CVD Confirmation");
    }

    // Отправка в матрицу (Binance Style Color Data)
    ...
    // ФИЛЬТР ВЫХОДА СИГНАЛА: Повышаем планку до 80 баллов
    if (score >= 80) {
        const type = currentTrend;
        
        // УСИЛЕННЫЙ АНТИ-СПАМ: 10 МИНУТ (600000 мс)
        ...
        const signalData = {
            ...
            tp: d.price * (type === "LONG" ? 1.015 : 0.985), // Тейк 1.5% (RR 1:3)
            score, 
            reason: reasons.join(" | ")
        };
        ...
        console.log(`>> [SNIPER_SIGNAL]: ${s.toUpperCase()} ${type} | SCORE: ${score}`);
    }
}
...
function connectWebSocket() {
    ...
    ws.on('open', () => console.log(">> [BINANCE_OS]: STREAM_ACTIVE"));
    ...
    // Ресет счетчика Velocity каждую секунду
    setInterval(() => {
        symbols.forEach(s => {
            if (market[s]) {
    ...
    ws.on('close', () => {
        console.log("!! [ALERT]: WS_CLOSED. REBOOTING...");
...
setInterval(() => {
    io.emit('system_heartbeat', {
        ...
        load: (Math.random() * 10 + 2).toFixed(1),
...
server.listen(PORT, () => {
    console.log(`
    JSCULPTOR SNIPER v7.1 // BINANCE PRO
    PORT: ${PORT} | NODES: 20
    FILTER: TREND_GUARD + ANTI_SPAM ENABLED
    `);
});