require('dotenv').config();
const express = require('express');
const http = require('http');
const { Server } = require('socket.io');
const WebSocket = require('ws');
const mongoose = require('mongoose');
const path = require('path');

const app = express();
const server = http.createServer(app);
const io = new Server(server);

const PORT = process.env.PORT || 3000;
const ACCESS_CODE = process.env.ACCESS_CODE || "JSculptor2026";

// --- DATABASE CONNECTION ---
mongoose.connect(process.env.MONGO_URI)
    .then(() => console.log(">> [BINANCE_SCALPER]: DB_CONNECTED_STABLE"))
    .catch(err => console.error("!! [DB_ERROR]:", err));

const Signal = mongoose.model('Signal', new mongoose.Schema({
    symbol: String,
    type: String,
    entry: Number,
    sl: Number,
    tp: Number,
    score: Number,
    reason: String,
    timestamp: { type: Date, default: Date.now }
}));

// --- MIDDLEWARE & PWA ---
app.use(express.static(path.join(__dirname)));
app.use(express.json());

app.get('/manifest.json', (req, res) => {
    res.sendFile(path.join(__dirname, 'manifest.json'));
});

// API Истории (24/7 доступ к сигналам)
app.get('/api/signals', async (req, res) => {
    try {
        const history = await Signal.find().sort({ timestamp: -1 }).limit(30);
        res.json(history);
    } catch (err) {
        res.status(500).json({ error: "HISTORY_FETCH_FAILED" });
    }
});

// --- SCALPER MARKET STATE CLASS ---
class MarketState {
    constructor() {
        this.price = 0;
        this.prevPrice = 0;
        this.candles = [];
        this.deltaWindow = [];
        this.currentMinuteDelta = 0;
        this.volumeSMA = 0;
        this.ema8 = 0;
        this.ema21 = 0;
        this.swings = { high: null, low: null };
        this.depth = { bids: 0, asks: 0 };
        // Новое для Скальпера: Скорость ленты
        this.tradeCount = 0;
        this.velocity = 0; 
    }

    updateIndicators() {
        if (this.candles.length < 21) return;
        const prices = this.candles.map(c => c.close);
        const calculateEMA = (data, period) => {
            const k = 2 / (period + 1);
            return data.reduce((acc, val) => val * k + acc * (1 - k), data[0]);
        };
        this.ema8 = calculateEMA(prices, 8);
        this.ema21 = calculateEMA(prices, 21);
        this.volumeSMA = this.candles.slice(-20).reduce((a, b) => a + b.volume, 0) / 20;
    }

    updateSwings(price) {
        if (!this.swings.high || price > this.swings.high) this.swings.high = price;
        if (!this.swings.low || price < this.swings.low) this.swings.low = price;
    }

    get rollingCVD() {
        return this.deltaWindow.reduce((a, b) => a + b, 0) + this.currentMinuteDelta;
    }
}

// --- 20 NODES ---
const symbols = [
    'btcusdt', 'ethusdt', 'solusdt', 'bnbusdt', 'dogeusdt', 'xrpusdt', 
    'adausdt', 'maticusdt', 'dotusdt', 'ltcusdt', 'shibusdt', 'trxusdt', 
    'avaxusdt', 'linkusdt', 'atomusdt', 'uniusdt', 'etcusdt', 'bchusdt', 
    'nearusdt', 'filusdt'
];

let market = {};
symbols.forEach(s => { market[s] = new MarketState(); });

// --- AGGRESSIVE SCALP ENGINE v7.0 ---
async function analyzeMarket(s) {
    const d = market[s];
    if (!d || d.candles.length < 5 || !d.depth.bids) return;

    let score = 0;
    let reasons = [];
    const cvd = d.rollingCVD;

    // 1. Агрессивный Скоринг Объема
    const last = d.candles[d.candles.length - 1];
    if (last && last.volume > d.volumeSMA * 1.8) {
        score += 25; reasons.push("Volume Surge");
    }

    // 2. Скорость Ленты (Velocity Spike)
    if (d.velocity > 50) { // Если более 50 сделок за такт
        score += 20; reasons.push("Tape Velocity High");
    }

    // 3. Анализ Ликвидности (SFP)
    if (d.swings.high && d.price > d.swings.high && cvd < 0) {
        score += 25; reasons.push("Liquidity Sweep");
    } else if (d.swings.low && d.price < d.swings.low && cvd > 0) {
        score += 25; reasons.push("Liquidity Sweep");
    }

    // 4. Дисбаланс стакана
    const imb = (d.depth.bids - d.depth.asks) / (d.depth.bids + d.depth.asks);
    if (Math.abs(imb) > 0.4) {
        score += 15; reasons.push("Orderbook Imbalance");
    }

    // 5. CVD Pressure
    if (Math.abs(cvd) > d.volumeSMA * 0.5) {
        score += 15; reasons.push("CVD Momentum");
    }

    // Отправка в матрицу (Binance Style Color Data)
    io.emit('matrix_update', { 
        s: s.toUpperCase(), 
        p: d.price, 
        d: cvd.toFixed(2), 
        sc: score,
        change: d.price > d.prevPrice ? 'up' : 'down'
    });
    d.prevPrice = d.price;

    // СИГНАЛ: Порог снижен до 65 для частоты скальпинга
    if (score >= 65) {
        const type = cvd > 0 ? "LONG" : "SHORT";
        
        // Анти-спам: не более одного сигнала в 2 минуты по одной монете
        const lastSig = await Signal.findOne({ symbol: s.toUpperCase() }).sort({ timestamp: -1 });
        if (lastSig && (Date.now() - lastSig.timestamp < 120000)) return;

        const signalData = {
            symbol: s.toUpperCase(), 
            type, 
            entry: d.price,
            sl: d.price * (type === "LONG" ? 0.996 : 1.004), // Короткий стоп 0.4%
            tp: d.price * (type === "LONG" ? 1.008 : 0.992), // Тейк 0.8%
            score, 
            reason: reasons.join(" | ")
        };

        const saved = await Signal.create(signalData);
        io.emit('new_signal', saved);
        console.log(`>> [SCALP_CONFIRMED]: ${s.toUpperCase()} ${type} @ ${d.price}`);
    }
}

// --- BINANCE ENGINE ---
function connectWebSocket() {
    const streams = symbols.map(s => `${s}@aggTrade/${s}@depth20@100ms/${s}@kline_1m`).join('/');
    const ws = new WebSocket(`wss://fstream.binance.com/ws/${streams}`);

    ws.on('open', () => console.log(">> [BINANCE_OS]: STREAM_ACTIVE"));

    ws.on('message', (data) => {
        const msg = JSON.parse(data);
        const s = (msg.s || "").toLowerCase();
        if (!s || !market[s]) return;
        const st = market[s];

        if (msg.e === 'aggTrade') {
            st.price = parseFloat(msg.p);
            st.currentMinuteDelta += msg.m ? -parseFloat(msg.q) : parseFloat(msg.q);
            st.tradeCount++; // Считаем тики
            st.updateSwings(st.price);
        } else if (msg.e === 'kline' && msg.k.x) {
            st.candles.push({ 
                close: parseFloat(msg.k.c), open: parseFloat(msg.k.o), 
                high: parseFloat(msg.k.h), low: parseFloat(msg.k.l), volume: parseFloat(msg.k.v) 
            });
            if (st.candles.length > 50) st.candles.shift();
            st.deltaWindow.push(st.currentMinuteDelta);
            if (st.deltaWindow.length > 10) st.deltaWindow.shift();
            st.currentMinuteDelta = 0;
            st.updateIndicators();
        } else if (msg.b) {
            st.depth.bids = msg.b.slice(0, 5).reduce((a, b) => a + parseFloat(b[1]), 0);
            st.depth.asks = msg.a.slice(0, 5).reduce((a, b) => a + parseFloat(b[1]), 0);
        }
        analyzeMarket(s);
    });

    // Ресет счетчика Velocity каждую секунду
    setInterval(() => {
        symbols.forEach(s => {
            if (market[s]) {
                market[s].velocity = market[s].tradeCount;
                market[s].tradeCount = 0;
            }
        });
    }, 1000);

    ws.on('close', () => {
        console.log("!! [ALERT]: WS_CLOSED. REBOOTING...");
        setTimeout(connectWebSocket, 3000);
    });
}

connectWebSocket();

// --- 24/7 KEEP-ALIVE ---
setInterval(() => {
    io.emit('system_heartbeat', {
        status: "SCALPER_ACTIVE",
        load: (Math.random() * 10 + 2).toFixed(1),
        uptime: process.uptime().toFixed(0)
    });
}, 5000);

// --- AUTH ---
app.post('/api/auth', (req, res) => {
    if (req.body.code === ACCESS_CODE) res.json({ ok: true });
    else res.status(401).json({ ok: false });
});

server.listen(PORT, () => {
    console.log(`
    SCALP HUNTER v7.0 // BINANCE EDITION
    PORT: ${PORT} | NODES: 20
    STATUS: 24/7 MONITORING ENABLED
    `);
});