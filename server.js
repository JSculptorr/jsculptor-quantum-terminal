...
mongoose.connect(process.env.MONGO_URI)
    ...
    .catch(err => console.error("!! SYSTEM: DB_ERROR", err));
...
// Специальный маршрут для PWA (чтобы приложение можно было скачать на телефон)
...
function connectWebSocket() {
    ...
    ws.on('error', (err) => console.error("!! WS_ERROR:", err));
    
    // Авто-реконнект при обрыве связи
    ws.on('close', () => {
        console.log(">> SYSTEM: WS_CONNECTION_LOST. RECONNECTING...");
...
server.listen(PORT, () => console.log(`Institutional Hacker Terminal V4 Live on port: ${PORT}`));