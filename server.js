...
async function analyzeMarket(s) {
    ...
    // Финальное обновление матрицы с полным скорингом
...
function connectWebSocket() {
    ...
    ws.on('close', () => {
        console.log("!! [SYSTEM]: CONNECTION_LOST. INITIATING_RECONNECT...");
...
// --- DECORATIVE SYSTEM PULSE ---
// Шлем на фронтенд «хакерские» статы, чтобы декоративные блоки жили
setInterval(() => {
    io.emit('system_heartbeat', {
        cpu: (Math.random() * 20 + 10).toFixed(1),
        net: (Math.random() * 100 + 50).toFixed(0),
        threat: "STABLE"
    });
}, 3000);
...
app.post('/api/auth', (req, res) => {
    if (req.body.code === ACCESS_CODE) {
        console.log(">> [AUTH]: ACCESS_GRANTED_FOR_NODE");
        res.json({ ok: true });
    } else {
        console.log("!! [AUTH]: UNAUTHORIZED_BREACH_ATTEMPT");
...
server.listen(PORT, () => {
    console.log(`
    ==========================================
    JSCULPTOR TERMINAL MONOLITH V5
    STATUS: ONLINE
    PORT: ${PORT}
