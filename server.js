...
mongoose.connect(process.env.MONGO_URI)
    .then(() => console.log(">> [SYSTEM_OS]: DATABASE_ESTABLISHED"))
    .catch(err => console.error("!! [CRITICAL_ERROR]: DB_CONNECTION_FAILED", err));
...
// Маршрут для PWA приложения
app.get('/manifest.json', (req, res) => {
    res.sendFile(path.join(__dirname, 'manifest.json'));
});

// --- API ДЛЯ ПОЛУЧЕНИЯ ИСТОРИИ (ЧТОБЫ ДАННЫЕ НЕ ПРОПАДАЛИ) ---
app.get('/api/signals', async (req, res) => {
    try {
        const history = await Signal.find().sort({ timestamp: -1 }).limit(20);
        res.json(history);
    } catch (err) {
...
// --- ADVANCED MARKET STATE CLASS (БЕЗ ИЗМЕНЕНИЙ ЛОГИКИ) ---
class MarketState {
    constructor() {
        this.price = 0;
...
// --- INSTITUTIONAL ENGINE (БЕЗ ИЗМЕНЕНИЙ ЛОГИКИ) ---
async function analyzeMarket(s) {
    ...
    if (last && last.volume > d.volumeSMA * 2.5 && Math.abs(last.close - last.open) < (last.high - last.low) * 0.3) {
        score += 35; reasons.push("Institutional Absorption");
    }
    ...
    if (Math.abs(imb) > 0.5) { score += 15; reasons.push("OB Imbalance"); }
    if (Math.abs(cvd) > d.volumeSMA * 0.7) { score += 20; reasons.push("Heavy CVD Pressure"); }
...
function connectWebSocket() {
    ...
    ws.on('open', () => console.log(">> [BINANCE]: DATA_LINK_ESTABLISHED"));
    ws.on('message', (data) => {
        ...
        if (msg.e === 'aggTrade') {
            ...
            if (msg.k.x) {
                st.candles.push({ close: parseFloat(msg.k.c), open: parseFloat(msg.k.o), high: parseFloat(msg.k.h), low: parseFloat(msg.k.l), volume: parseFloat(msg.k.v) });
    ...
    ws.on('close', () => {
        console.log("!! [SYSTEM]: CONNECTION_LOST. RECONNECTING...");
        setTimeout(connectWebSocket, 5000);
    });
}
connectWebSocket();

// --- DIGITAL HACKER HEARTBEAT ---
setInterval(() => {
    io.emit('system_heartbeat', {
        cpu: (Math.random() * 15 + 5).toFixed(1),
        net: (Math.random() * 500).toFixed(0),
...
server.listen(PORT, () => {
    console.log(`
    ==========================================
    JSCULPTOR MONOLITH V5 // DEPLOYED
    NODES: ${symbols.length} ACTIVE
    STATUS: SYSTEM_READY
    ==========================================
    `);
});